...
import { DELETE_AGENDA_ITEM_FOLDER_MUTATION } from 'GraphQl/Mutations/AgendaFolderMutation';
...
function AgendaFolderContainer({
  ...
  const updateAgendaFolderHandler = async (
    ...
    try {
      ...
      refetchAgendaFolder();
      hideUpdateModal();
      toast.success(t('agendaFolderUpdated') as string);
    } catch (error: unknown) {
      if (error instanceof Error) {
        toast.error(`Agenda folder Update Failed ${error.message}`);
      }
    }
  };
  ...
  const deleteAgendaFolderHandler = async (): Promise<void> => {
    try {
      ...
      refetchAgendaFolder();
      toggleDeleteItemModal();
      toast.success(t('agendaFolderDeleted') as string);
    } catch (error) {
      if (error instanceof Error) {
        toast.error(`${error.message}`);
      }
  ...
  const onItemDragEnd = async (result: DropResult): Promise<void> => {
    ...
    try {
      ...
      toast.success(`Item sequence updated successfully`);
      refetchAgendaFolder();
    } catch (error) {
      if (error instanceof Error) {
        toast.error(error.message);
  ...
  const onFolderDragEnd = async (result: DropResult): Promise<void> => {
    ...
    try {
      ...
      toast.success(`Section sequence updated successfully`);
